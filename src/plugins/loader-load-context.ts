...
function buildCacheKey(params: {
  ...
  const cacheIdentity = {
    ...
    plugins: { ...params.plugins, loadPaths },
    installs,
...
export function resolvePluginLoadCacheContext(options: PluginLoadOptions = {}) {
  ...
  return {
    ...
    installRecords,
    devSourceRoot,
    // Metadata and private runtime snapshots have different API capabilities.
    cacheKey: options.mode === "cli-metadata" ? `cli-metadata::${cacheKey}` : cacheKey,
  };
}
